...
The browser opens the first one. To see the clock, click the link on the
...
In particular, a desktop program will NOT run here: anything that opens
a window - RingQt, GUILib, forms, dialogs, drawing. Those paint on a
desktop window through Qt, which does not exist in a browser.

But the same IDEA works on the web. A desktop clock program cannot be
loaded and run here - yet clock.html in this folder IS an analog clock,
hands and all, with its logic written in Ring. The face is drawn in the
page; Ring works out where the hands should point.

That is the pattern: what carries over is your Ring - the logic, the
rules, the calculations. What changes is the screen it draws on.
