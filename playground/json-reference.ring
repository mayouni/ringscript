# json-reference.ring — the PURE-RING JSON codec, under renamed entry
# points, so a page can time it beside the C codec that replaced it.
#
# This is src/ringlib/json.ring verbatim except for the two function names.
# It still ships inside the runtime as the reference the C codec is held
# byte-identical to (a permanent gate compares them), and native Ring runs
# this one. Loading it here is what lets ledger.html show a real
# stock-Ring column instead of a claim.
